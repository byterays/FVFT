...
<ul class="nav nav-tabs nav-justified navtab-wizard tabItems bg-muted"
    ...
    <li class="nav-item {{ $_editProfile || $_changePassword || $_deleteAccount ? 'active' : '' }}"><a href="{{ route('candidate.account_setting.index') }}" class="nav-link font-bold">{{ strtoupper('Account Setting') }}</a>
    </li>
    ...
    <li class="nav-item {{ !$_support ?: 'active' }}"><a href="{{ route('candidate.support.index') }}" class="nav-link font-bold">{{ strtoupper('Support') }}</a></li>
